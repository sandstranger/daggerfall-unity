...
namespace DaggerfallWorkshop.Game
{
    ...
    public class PlayerMotor : MonoBehaviour
    {
        ...
        public bool StartRestGroundedCheck()
        {
            ...
            return (Physics.Raycast(ray, controller.height / 2 + 0.2f, DFULayerMasks.CorporealMask));
        }
        ...
        public Vector3 FindGroundPosition(float distance = 10)
        {
            RaycastHit hit;
            Ray ray = new Ray(transform.position, Vector3.down);
            if (Physics.Raycast(ray, out hit, distance, DFULayerMasks.CorporealMask))
                return hit.point;
        ...
        public bool FixStanding(float extraHeight = 0, float extraDistance = 0)
        {
            ...
            if (Physics.Raycast(ray, out hit, (controller.height * 2) + extraHeight + extraDistance, DFULayerMasks.CorporealMask))
            {
        ...
        bool GetOnExteriorGroundMethod()
        {
            ...
            RaycastHit hit;
            if (GameManager.Instance.PlayerEnterExit.IsPlayerInside || !Physics.Raycast(transform.position, Vector3.down, out hit, rayDistance * 2, DFULayerMasks.CorporealMask))
            {
                return false;
            }
            else
